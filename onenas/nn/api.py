...
class InputChoice(nn.Module):
    ...
    * ``concat``: concatenate all chosen inputs at dimension 1.
    ...
    def __new__(cls, n_candidates: int, n_chosen: int = 1, reduction: str = 'sum',
                label: Optional[str] = None, **kwargs):
        try:
    ...
    def __init__(self, n_candidates: int, n_chosen: int = 1, reduction: str = 'sum',
                 label: Optional[str] = None, **kwargs):
...
class ValueChoice(Translatable, nn.Module):
    ...
    def access(self, value):
        ...
        try:
            ...
            raise KeyError(
                ''.join([f'[{a}]' for a in self._accessor]) + f' does not work on {value}')
        return v
...
class ChosenInputs(nn.Module):
    ...
    def _tensor_reduction(self, reduction_type, tensor_list):
        ...
        if reduction_type == 'concat':
            ...
            # TODO:return torch.cat(tensor_list, dim=1)
