    ...
    def mutate(self, model):
        ...
        for node in self.nodes:
            ...
            target.update_operation('__torch__.nni.retiarii.nn.pytorch.ChosenInputs',
                                    {'chosen': chosen,
                                     'reduction': node.operation.parameters['reduction']})
...
class ValueChoiceMutator(Mutator):
    ...
    def mutate(self, model):
        ...
        for node in self.nodes:
            target = model.get_node_by_name(node.name)
            target.update_operation('prim::Constant',
                                    {'type': type(chosen).__name__, 'value': chosen})
...
class ParameterChoiceMutator(Mutator):
    ...
    def mutate(self, model):
        ...
        for node, argname in self.nodes:
            ...
            target = model.get_node_by_name(node.name)
            target.update_operation(target.operation.type,
                                    {**target.operation.parameters, argname: chosen_value})
    ...
    def _retrieve_chain_from_graph(self, graph: Graph) -> List[Node]:
        ...
        while u != graph.output_node:
            if u != graph.input_node:
                chain.append(u)
            assert len(
                u.successors) == 1, f'This graph is an illegal chain. {u} has output {u.successor}.'
            u = u.successors[0]
        return chain
...
def process_inline_mutation(model: Model) -> Optional[List[Mutator]]:
    ...
    ic_nodes = _group_by_label(
        model.get_nodes_by_type('__torch__.nni.retiarii.nn.pytorch.api.InputChoice'))
    for node_list in ic_nodes:
        assert _is_all_equal(
            map(lambda node: node.operation.parameters['n_candidates'], node_list)) and \
    ...
    vc_nodes = _group_by_label(
        model.get_nodes_by_type('__torch__.nni.retiarii.nn.pytorch.api.ValueChoice'))
    ...
    for node_list in pc_nodes:
        assert _is_all_equal(
            [node.operation.parameters[name].candidates for node, name in node_list]), \
            'Value choice with the same label must have the same candidates.'
        first_node, first_argname = node_list[0]
        mutator = ParameterChoiceMutator(node_list,
                                         first_node.operation.parameters[first_argname].candidates)
        applied_mutators.append(mutator)

    # apply layer choice at last as it will delete some nodes
    lc_nodes = _group_by_label(
        filter(lambda d: d.operation.parameters.get('mutation') == 'layerchoice',
               model.get_nodes_by_type('_cell')))
    for node_list in lc_nodes:
        assert _is_all_equal(
            map(lambda node: len(node.operation.parameters['candidates']), node_list)), \
    ...
    for node_list in repeat_nodes:
        assert _is_all_equal(
            map(lambda node: node.operation.parameters['max_depth'], node_list)) and \
...
def extract_mutation_from_pt_module(pytorch_model: nn.Module) -> Tuple[
    Model, Optional[List[Mutator]]]:
    ...
    if len(inspect.signature(model.python_class.__init__).parameters) > 1:
        if not hasattr(pytorch_model, '_init_parameters'):
            raise ValueError(
                'Please annotate the model with @serialize decorator in python execution mode '
