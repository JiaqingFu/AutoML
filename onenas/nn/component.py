...
class Repeat(nn.Module):
    ...
    def __new__(cls, blocks: Union[
        Callable[[], nn.Module], List[Callable[[], nn.Module]], nn.Module, List[nn.Module]],
    ...
    def __init__(self,
                 blocks: Union[
                     Callable[[], nn.Module], List[Callable[[], nn.Module]], nn.Module, List[
                         nn.Module]],
                 depth: Union[int, Tuple[int, int]], label: Optional[str] = None):
        super().__init__()
    ...
    def _replicate_and_instantiate(blocks, repeat):
        ...
        assert len(blocks) > 0
        assert repeat <= len(
            blocks), f'Not enough blocks to be used. {repeat} expected, only found {len(blocks)}.'
...
class Cell(nn.Module):
        ...
        for i in range(num_nodes):
            ...
            for k in range(num_ops_per_node):
                ...
                self.inputs[-1].append(
                    InputChoice(i + num_predecessors, 1, label=f'{self.label}/input_{i}_{k}'))
    ...
    def forward(self, x: List[flow.Tensor]):
        ...
        for ops, inps in zip(self.ops, self.inputs):
            ...
            current_state = flow.sum(flow.stack(current_state), 0)
            states.append(current_state)
        return flow.cat(states[self.num_predecessors:], 1)
