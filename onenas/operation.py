...
class Operation:
    ...
    def new(type_name: str, parameters: Dict[str, Any] = {}, cell_name: str = None) -> 'Operation':
        if type_name == '_cell':
            ...
            elif debug_configs.framework.lower() in ('flow', 'of', 'oneflow'):
                from .operation_def import flow_op_def  # pylint: disable=unused-import
                cls = OneflowOperation._find_subclass(type_name)
    ...
    def __eq__(self, other):
        return type(other) is type(
            self) and other.type == self.type and other.parameters == self.parameters
...
class PyTorchOperation(Operation):
    ...
    def to_forward_code(self, field: str, output: str, inputs: List[str],
                        inputs_value: List[Any] = None) -> str:
        """
        Parameters
        ----------
        field : str
...
class OneflowOperation(Operation):
    @classmethod
    def _find_subclass(cls, subclass_name):
        if cls.to_class_name(subclass_name) is not None:
            subclass_name = 'ModuleOperator'
    ...
    @classmethod
    def to_class_name(cls, type_name) -> str:
        if type_name.startswith('__flow__.'):
            return type_name[len('__flow__.'):]
        elif type_name.startswith('__mutated__.'):
            return type_name[len('__mutated__.'):]
        else:
            return None
    ...
    def _to_class_name(self) -> str:
        if self.type.startswith('__flow__.'):
            return self.type[len('__flow__.'):]
        elif self.type.startswith('__mutated__.'):
            return self.type[len('__mutated__.'):]
        else:
            return None
    ...
    def get_import_pkg(self) -> str:
        if self.type.startswith('__flow__.'):
            return self.type[len('__flow__.'):].split('.')[0]
        elif self.type.startswith('__mutated__.'):
            return self.type[len('__mutated__.'):].split('.')[0]
        else:
            return None
